...
projectChatRouter.post("/", requireAuth, async (req, res) => {
  ...
  const messagesForLLM: ChatMessage[] = displayed_doc
    ? enrichedMessages.map((m, i) => {
        ...
        return {
          ...m,
          content: `${m.content}\n\ndisplayed_doc: ${sanitizeLlmInput(displayed_doc.filename)}, displayed_doc_id: ${displayed_doc.document_id}`,
        };
      })
    : enrichedMessages;
  ...
  if (attached_documents?.length) {
    ...
    const lines = attached_documents.map((d) => {
      ...
      return slug
        ? `- ${slug}: ${sanitizeLlmInput(d.filename)}`
        : `- ${sanitizeLlmInput(d.filename)}`;
    });
