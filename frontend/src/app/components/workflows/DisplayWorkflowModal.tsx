...
import { WritingAssistButton } from "../shared/WritingAssistButton";
...
function MarkdownBody({ content }: { content: string }) {
  return (
    <ReactMarkdown
      ...
      components={{
        h1: ({ children }) => (
          <h1 className="text-base font-semibold text-gray-900 mt-4 mb-1 first:mt-0">{children}</h1>
        ),
        h2: ({ children }) => (
          <h2 className="text-sm font-semibold text-gray-900 mt-3 mb-1 first:mt-0">{children}</h2>
        ),
        h3: ({ children }) => (
          <h3 className="text-xs font-semibold text-gray-900 mt-2 mb-0.5 first:mt-0">{children}</h3>
        ),
  ...
  return (
    <div className="flex-1 border-l border-t border-gray-200 flex flex-col overflow-hidden px-3 pb-3">
      ...
      <div className="flex-1 overflow-y-auto border border-gray-200 rounded-md bg-gray-50">
        {columns.length === 0 ? (
          <p className="px-4 py-6 text-xs text-center text-gray-400">No columns defined</p>
        ) : (
          columns.map((col) => {
                ...
                {isExpanded && (
                  <div className="px-4 py-3 bg-white border-t border-gray-200 text-sm text-gray-600 leading-relaxed font-serif space-y-3">
                    {col.tags && col.tags.length > 0 && (
                      <div>
                        <p className="text-xs font-medium text-gray-400 mb-1.5 font-sans">Tags</p>
                    ...
                    <div>
                      <p className="text-xs font-medium text-gray-400 mb-1 font-sans">Prompt</p>
          ...
          <div className="flex items-center gap-1.5 text-xs text-gray-400">
            {screen === "select" ? (
              ...
              <>
                ...
                <span>›</span>
                <span className="truncate max-w-[160px]">{wf.title}</span>
                <span>›</span>
            ...
            <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div className="px-5 pb-3 shrink-0">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-medium text-gray-700">Message (optional)</p>
                  <WritingAssistButton
                    contextType="supplementary_instructions"
                    getCurrentText={() => assistantPrompt}
                    onResult={(text) => setAssistantPrompt(text)}
                    metadata={{ workflow_title: wf.title }}
                    size="sm"
                  />
                </div>
                <textarea
