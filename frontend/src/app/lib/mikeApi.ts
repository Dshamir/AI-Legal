...
export async function renameProjectFolder(
  ...
  return apiRequest<MikeFolder>(`/projects/${projectId}/folders/${folderId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function moveDocumentToFolder(
  ...
  return apiRequest<MikeDocument>(`/projects/${projectId}/documents/${documentId}/folder`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function renameProjectDocument(
  ...
  return apiRequest<MikeDocument>(`/projects/${projectId}/documents/${documentId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function renameDocumentVersion(
  ...
  return apiRequest<MikeDocumentVersion>(`/single-documents/${documentId}/versions/${versionId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function uploadProjectDocument(projectId: string, file: File): Promise<MikeDocument> {
  const authHeaders = await getAuthHeader();
  const form = new FormData();
  form.append("file", file);
...
export async function uploadStandaloneDocument(file: File): Promise<MikeDocument> {
  const authHeaders = await getAuthHeader();
  const form = new FormData();
  form.append("file", file);
...
export async function streamTabularGeneration(reviewId: string): Promise<Response> {
  const authHeaders = await getAuthHeader();
...
export async function listWorkflows(type: WorkflowType): Promise<MikeWorkflow[]> {
...
export type WritingAssistContextType =
  | "workflow_prompt"
  | "column_extraction"
  | "supplementary_instructions";

export async function writingAssist(params: {
  context_type: WritingAssistContextType;
  current_text: string;
  ...
  return apiRequest<{ text: string }>("/writing-assist", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
